...
We talked about Moore's Law for hardware. In AI, a similar growth has been seen in the *training compute*. One metric is **[FLOPs](https://en.wikipedia.org/wiki/FLOPS)** (floating-point operations). Training early large language models required astronomical computation – on the order of 10^23 to 10^24 FLOPs. These numbers highlight that to get these emergent abilities, we are throwing an enormous amount of computation into training. The cost is significant (frontier models can cost tens to hundreds of millions of dollars to train in compute alone).
...
Policymakers are actively grappling with this. One more angle: **Open weights vs closed also matters for international security.** Closed models might be considered dual-use tech, maybe subject to export controls (like advanced chips are). If weights are open, they spread globally instantly (hard to control), giving the best open source models a potential first-mover advantage.
...
**[OpenAI](https://openai.com/):** Founded 2015 with a mission to build AGI (artificial general intelligence) for the benefit of humanity. Started as a non-profit, later added a for-profit "OpenAI LP" with capped returns to attract funding (Microsoft invested $10B+). Created ChatGPT and the GPT series of models. They are arguably on the cutting edge of large language models and reasoning systems. Known for iterative deployment (released models gradually with more safety research each time). CEO Sam Altman has been a prominent voice, even calling for regulation on frontier AI labs (including possibly regulating OpenAI itself). OpenAI has restructured as a Benefit Corporation, while retaining the non-profit as part of its broader ownership structure.

**[Google DeepMind](https://deepmind.google/):** A UK-based lab acquired by Google in 2014. Famous for AlphaGo (the system that beat the Go world champion), [AlphaFold](https://alphafold.ebi.ac.uk/) (protein folding solution), and many core research contributions (reinforcement learning, etc.). In 2023, Google merged DeepMind with its Google Brain team to concentrate talent, now called Google DeepMind. They develop the Gemini series of multimodal models. DeepMind has historically put strong emphasis on ethics and safety: they had an ethics board from early on (though not public) and have done work on technical safety. They also pioneered some interpretability work. Google/DeepMind is in a tricky spot: needing to deploy AI in products (search, cloud) to compete, while also having many staff aware of and vocal about safety issues.

**[Anthropic](https://www.anthropic.com/):** Founded 2021 by OpenAI alumni who left after disagreements (particularly after OpenAI went for-profit and took the Microsoft deal). Led by Dario Amodei. They position themselves as an "AI safety and research company" – basically wanting to be the lab that will solve alignment. They've developed the Claude chatbot models, which are highly competitive. They introduced Constitutional AI. They've been generally more open about their safety research, but not fully open with model weights (Claude is via API). They've gotten substantial funding (billions of dollars, including Google and Amazon as investors). Anthropic has testified in US Congress about bio-risk of AI. They advocate for more oversight of powerful models. Their motto is to scale AI cautiously. They may welcome certain regulation that forces all labs to slow down or adhere to safety norms (level playing field for them, since safety is their brand).

**Others:**

- **Microsoft:** Not just a partner to OpenAI, Microsoft has its own AI R&D but lately they've chosen to integrate OpenAI tech deeply: Bing Chat, Microsoft 365 Copilot, Azure OpenAI services. They're basically a commercialization arm for OpenAI's models. They benefit from selling the cloud compute that trains these models (Azure). They also invested in hardware. Microsoft CEO Nadella is very bullish on "AI as co-pilot for every user". They are presumably also thinking of enterprise AI, ensuring corporate data remains private etc.

- **Meta (Facebook):** They took a different route by open-sourcing their big LLaMA models. They believe an open ecosystem is more innovative (and it undermines competitors' proprietary advantage). They also use AI extensively in their platforms (content moderation, feeds) and have their own cutting-edge research (they had a top translation model, image generation, etc.). CEO Zuckerberg seems less concerned about existential risk, more about keeping up and deploying AI widely (and on-device AI). Meta's open approach caused controversy as mentioned, but also earned goodwill from researchers. They explicitly put out LLaMA models "for commercial use" with some usage guidelines.

- **[xAI](https://x.ai/):** Founded by Elon Musk, xAI develops the Grok series of models and has rapidly scaled to become a frontier AI lab, with significant compute resources and competitive model performance.

- **China's AI efforts:** Companies like Baidu, Alibaba, Huawei, Tencent and DeepSeek have all developed large models. DeepSeek has demonstrated impressive technical capabilities, particularly in reasoning models, despite operating under hardware constraints from export controls. The Chinese government is heavily funding AI, but also regulating content (e.g., generative AI must align with Chinese values, and companies must register their models). Internationally, if Western companies restrict some uses, some fear users could just use Chinese models that have different rules. It's a geopolitically competitive field (the release of DeepSeek's competitive models was seen by some as a "Sputnik moment," fueling national AI strategies).

- **Hardware:** [Nvidia](https://www.nvidia.com/) has essentially a monopoly on high-end AI training chips (H100, H200, and Blackwell series GPUs). Every large model needs thousands of Nvidia GPUs. Their market capitalization soared, making them one of the world's most valuable companies due to AI demand. [TSMC](https://www.tsmc.com/) in Taiwan manufactures those chips and almost all advanced chips (making it strategically important; US has been trying to restrict advanced chip exports to slow certain countries' military AI progress). Manufacturing advanced AI chips requires [extreme ultraviolet (EUV) lithography](https://en.wikipedia.org/wiki/Extreme_ultraviolet_lithography) machines, which are only made by [ASML](https://www.asml.com/) in the Netherlands—a critical chokepoint in the global semiconductor supply chain. Intel and AMD are trying to get into AI accelerators too. Also, some labs (Google with TPUs, Amazon with AWS Trainium, etc.) have their own chips. But Nvidia is still generally ahead due to their strong software ecosystem (CUDA).

- **Cloud providers:** AI training happens on big cloud data centers. The "compute" is concentrated in US (mostly on Azure, AWS, Google Cloud) and some in China. This concentration has led to ideas like maybe track compute usage as a proxy to monitor if someone's training a super-powerful model (since if someone suddenly starts using tens of thousands of GPUs you know something's up).

It's clear that only a few firms or alliances can build the very top models because of the resources needed (data, talent, and especially compute). The "frontier model" club includes maybe a dozen entities worldwide. This concentration has several implications. One is a potential **safety advantage**: it is easier to coordinate and implement best practices among a small number of players (and indeed many of these companies are talking to each other and to governments about safety, even if they are competitors). Another implication, however, is **market concentration**: will access to advanced AI be limited to those who can afford API fees or who are in certain countries? Open-source efforts push back on this by broadening access, but even they often rely on reproducing last year's models, not the very latest. Policymakers are thus concerned with both fostering competition (so no single company monopolizes AI, which could stifle innovation or drive up costs) and preventing dangerous proliferation (so that extremely powerful models don't get into the wrong hands too easily).

Policymakers might leverage this structure: e.g., if wanting to enforce something, they might target cloud providers or chip companies (like export controls did with chips). Or collaborate with the leading labs which have expertise – e.g., some governments have advisory boards with AI company leaders.

We should mention regulatory bodies forming: In the US, there's been talk of an AI regulator. The UK set up an [AI Safety Institute](https://www.aisi.gov.uk/). The EU will empower existing agencies to enforce the AI Act (and maybe create a new board). The landscape of "who oversees the industry" is fluid.

**Industry orgs:** There are partnerships like [Partnership on AI](https://partnershiponai.org/) (multistakeholder group including companies, NGOs), and the [Frontier Model Forum](https://www.frontiermodelforum.org/) (OpenAI, Anthropic, Google, Microsoft formed this to jointly work on safety standards for advanced models). This shows even competitors know cooperation is needed on certain issues (like sharing best practices for red-teaming or safety evals).

### Regulatory Efforts

Regulatory efforts are now underway. The [EU AI Act](https://artificialintelligenceact.eu/), which began coming into force in 2024-2025, is one of the first comprehensive frameworks. It uses a risk-based approach: applications of AI are classified by risk level (unacceptable risk applications are banned, high-risk ones have strict requirements, etc.). Providers of foundation models (like GPT-type models) must conduct risk assessments, document their training data and methods, and implement safety measures. They may have to register in an EU database and agree to a "Code of Practice" that includes adherence to EU copyright law. Full enforcement begins in August 2026.

In the United States, the approach has evolved. Leading AI firms – OpenAI, Google, Microsoft, etc. – made voluntary commitments to invest in cybersecurity, share information about AI risks with the government, and develop tech like watermarking for AI-generated content. NIST (the National Institute of Standards and Technology) released an [AI Risk Management Framework](https://www.nist.gov/itl/ai-risk-management-framework) to guide companies in identifying and mitigating risks.

The Trump Administration restructured its approach to AI oversight by establishing the Center for AI Standards and Innovation (CAISI). Under the administration, CAISI's mandate is to develop testing standards, benchmarks, and evaluation protocols for AI systems, in collaboration with industry. This came alongside policy changes emphasizing innovation and deregulation at the federal level. The administration released "Winning the Race: America's AI Action Plan" in July 2025, a comprehensive strategy document covering AI innovation, infrastructure, and international competitiveness. We're essentially seeing a move towards standardization and best practices as the cornerstone of US AI governance, rather than broad new laws.

**Key Takeaways (Chapter 9):**

- **Frontier AI development is concentrated** in a handful of private labs and big tech firms, mostly in the US (OpenAI/Microsoft, Google/DeepMind, Meta, Anthropic, xAI) with emerging strong efforts in China and a spread of smaller players. This concentration means policy engagement with a few key companies can have wide impact (e.g., getting them to agree to standards).

- **Different philosophies:** OpenAI (controlled release, seeking AGI), DeepMind (research-heavy, now more product-focused under Google), Anthropic (safety-focused, transparency leaning), Meta (open-source advocacy), Microsoft (fast deployment to consumers, enterprise integration), Google (guarded due to brand risk but cannot fall behind in search AI). These differences might influence how they respond to regulation – e.g., Meta might push back on rules that limit open models, OpenAI/Microsoft might push for licensing that they can meet but newcomers can't, etc.

- **Compute/hardware as strategic assets:** Nvidia and TSMC are as critical to AI as any lab – shortages or export bans can limit who advances. E.g., US restrictions on chips to China aim to slow Chinese training of huge models. Nations are investing in domestic AI chip production to reduce such dependencies (US [CHIPS and Science Act](https://en.wikipedia.org/wiki/CHIPS_and_Science_Act), EU's chip initiatives). The lithography equipment bottleneck (ASML) adds another layer of strategic complexity.

- **International dynamic:** There is a legitimate race – not just corporate but national – to harness AI's economic and military advantages. This complicates global alignment on safety: if one country's companies slow down for safety and another's don't, the latter could leap ahead (or at least that perception can cause reluctance to pump brakes). How to bring Chinese players into a global safety fold is an open question. Possibly areas of common interest like avoiding AI-assisted bio-terror or accidental escalation could drive cooperation.

- **Public-private partnership:** Many leading experts are in industry, not government or academia, due to the resource needs of AI research. Regulators will likely rely on those companies for expertise – but also must be cautious of conflicts of interest. E.g., inviting companies to help write rules (good for practicality, but they may skew it to their interests). Striking that balance is key.
...
**Effectiveness and challenges:** Export controls have clearly constrained Chinese AI development, forcing reliance on older chips and domestic alternatives that lag behind. By some measures, the U.S. now has a nearly 30x compute advantage over China in aggregate. However, enforcement remains difficult. Chips are small and valuable, creating smuggling incentives. Third-country transshipment is hard to monitor—chips sold to Malaysia or Singapore may find their way to China. And Chinese firms stockpiled chips ahead of restrictions, with some estimates suggesting years of supply for major labs. The controls buy time but aren't a permanent solution. They work best as part of a broader strategy that also invests in American capabilities rather than relying solely on slowing competitors.
...
- **Export controls** are the primary US tool for maintaining AI advantage, targeting chips and manufacturing equipment through the Entity List mechanism. They've constrained competitors but face enforcement challenges including smuggling and stockpiling.
...
- **State laws are proliferating:** California's SB 53, New York's RAISE Act, and narrower laws across states create a patchwork that industry finds challenging. Some laws, like Illinois's AI therapy ban, are poorly designed. Federal preemption remains elusive.
...
**Hardware constraints:** Chinese AI labs can no longer legally access Nvidia's most advanced chips. The H100 and subsequent generations are banned; even the A800 and H800 (chips Nvidia designed specifically to comply with earlier rules) were subsequently restricted. The April 2025 H20 ban closed another loophole—Nvidia's lower-performance chip designed for the Chinese market. Chinese firms must now rely on stockpiled chips, older generations, and domestic alternatives.

**Stockpiling evidence:** Chinese tech giants anticipated restrictions and built substantial reserves. In 2023, [Baidu, ByteDance, Tencent, and Alibaba placed combined orders worth $5 billion](https://www.datacenterdynamics.com/en/news/chinas-tech-giants-order-5bn-in-nvidia-chips-including-100000-a800-gpus-for-2023/) for Nvidia chips. Before the H20 ban took effect, these same companies rushed to acquire an estimated 1-1.6 million H20 chips worth $12-16 billion—enough for 12-14 months of inference workloads. Chinese companies also imported over $26 billion in semiconductor manufacturing equipment in the first seven months of 2024, quadrupling imports of Dutch lithography equipment before restrictions tightened. These stockpiles provide runway, but not a permanent solution.

**Workarounds:** Despite controls, chips continue reaching China through smuggling, transshipment via third countries, and front companies. The high value-to-weight ratio makes enforcement difficult—a single suitcase can carry millions of dollars worth of computing power. As of late 2024, approximately 75% of chips powering AI training in Chinese data centers remained Nvidia/CUDA-based, suggesting both the scale of stockpiles and the limits of domestic alternatives.
...
**Ascend chips:** The Ascend 910C, Huawei's current flagship AI accelerator, delivers 780-800 TFLOPS in FP16/BF16—roughly 60-80% of Nvidia's H100 performance. It combines two 910B dies via chiplet packaging, with 128GB of HBM3 memory. Yield rates have improved from 20-30% initially to approximately 40%, with targets of 60% by 2025. Huawei aims to produce 100,000 910C units and 300,000 910B units in 2025. An Ascend 910D on 5nm process is anticipated for 2026.
...
**Technology denial has limits:** Export controls have bought time but won't prevent Chinese AI development. China has the talent, capital, and increasingly the domestic supply chains to continue advancing. The stockpiles provide 1-2 years of runway; indigenous capabilities are improving. The relevant question is whether the US maintains a meaningful and durable lead, or whether China achieves rough parity through efficiency innovations and domestic alternatives.

**Key Takeaways (Chapter 11):**

- **Export controls have constrained but not stopped** Chinese AI development. Stockpiles worth billions provide runway while indigenous capabilities mature.

- **DeepSeek demonstrated** that efficiency innovations can partially offset hardware disadvantages, achieving frontier performance with dramatically lower costs.

- **Huawei is building a parallel stack** from chips (Ascend) to software (CANN/MindSpore) to devices (HarmonyOS), reducing dependence on American technology.
...
**GitHub Copilot:** GitHub's AI coding assistant now has 20 million cumulative users, including 90% of Fortune 100 companies. Studies show 55% faster task completion and 75% reductions in pull request cycle times. About 46% of code in Copilot-enabled workflows is now AI-generated. However, a [rigorous METR study](https://metr.org/blog/2025-07-10-early-2025-ai-experienced-os-dev-study/) found that experienced open-source developers working on mature projects actually completed tasks 19% slower with AI tools—despite believing they were faster. Code "churn" (code discarded within two weeks) is projected to double, suggesting quality concerns.

**Cursor AI:** The fastest-growing SaaS company in history, reaching $100 million in annual recurring revenue within 12 months of launch and $500 million by May 2025. Its $29 billion valuation (November 2025) reflects investor conviction that AI-native development environments will dominate. Enterprise customers include OpenAI, Shopify, and Coinbase, where "single engineers are now refactoring, upgrading, or building new codebases in days instead of months."

**Devin and autonomous agents:** Cognition Labs' [Devin](https://www.cognition.ai/blog/introducing-devin) represents the next step: an AI that autonomously plans, writes, debugs, tests, and deploys code. It achieves 13.86% success on benchmark tasks with no human assistance—impressive compared to prior models but sobering in absolute terms. Real-world testing finds "only a handful of successes out of twenty tasks." The consensus: senior-level at codebase understanding but junior at execution.

**The METR task horizon:** The nonprofit METR measures how complex a task AI can complete autonomously. Their research finds that this "50% time horizon" has been doubling approximately every 7 months for six years—possibly accelerating to every 4 months in 2024. If the trend continues, frontier AI will handle month-long projects by the end of the decade. This exponential trajectory suggests current disruptions are just beginning.

### Companies and Layoffs

The 2024-2025 period saw the first large-scale layoffs explicitly citing AI.

**Major examples:** Amazon (14,000 jobs—its largest layoff ever), Microsoft (15,000, with CEO Nadella confirming 30% of code is now AI-written), UPS (48,000 via AI-enabled logistics), Salesforce (5,000, AI agents replacing support roles), and IBM (2,000-3,000 back-office positions). By late 2025, [over 55,000 US job cuts were attributed to AI](https://www.cnbc.com/2025/12/21/ai-job-cuts-amazon-microsoft-and-more-cite-ai-for-2025-layoffs.html) according to [Challenger, Gray & Christmas](https://www.challengergray.com/).
...
- **Coding is the leading indicator:** AI tools show 50%+ productivity gains in studies, but real-world effects are contested. The METR task horizon is doubling every 7 months—suggesting current disruptions are early-stage.
